...
void handleUserChoice(int choice, bool& exitProgram); // Processes the user's choice and navigates accordingly
...
int getUserChoice() {
    ...
    while (true) {
        ...
        if (cin.fail() || (choice < 0 || choice > 3)) {
            ...
            cout << "\033[91mInvalid input. Please enter a valid choice (0-3): \033[0m";
        } else {
            cin.ignore(numeric_limits<streamsize>::max(), '\n'); // Clear input buffer
